...
1. When a keyword list is edited, the old list version is **archived**. The
   inherited match records captured against the old version are **carried
   forward** to the current/active result set. Active hits always come back from
   fresh screening; inherited hits are carried, not recomputed.
2. For a `RISK_DATA_UPDATE` inherited hit, the stored `keywordListVersion` stays
   at the **old** version where the keyword still existed (`klv1`), even though
   the current list version is `klv2`.
3. **Stickiness / first-cause-wins.** Once a hit is captured as inherited, its
   ...
   create a new active hit.

---

## 2. Match-record schema

Each match record has these fields:

| Field | Example | Notes |
|---|---|---|
| `shipmentNumber` | `s1` | The screened shipment. |
| `shipmentVersion` | `v1` | Version of the shipment that this state reflects. |
| `keyword` | `k1` | The matched keyword. |
| `keywordList` | `kl1` | Always `kl1` in these scenarios. |
| `keywordListVersion` | `klv1` | For `RISK_DATA_UPDATE` this is the **old** version where the keyword existed. |
...
- **Shipment & screening info** — `{ shipmentNumber, shipmentVersion, keywordList, keywordListVersion, action }`
  where `action` ∈ `REGULAR_SCREENING | KEYWORD_LIST_UPDATE | SHIPMENT_UPDATE`.
- **Matches** — an array of match records representing the **full match state**
  for the shipment after that step.

---

## 3. Scenarios

### Scenario #1 : Regular hit (baseline)
...
### Scenario #2 : Shipment update removes the matched keyword → SHIPMENT_UPDATE
...
matches, so the hit is inherited as a shipment-driven removal.
...
### Scenario #3 : Keyword removed from list → RISK_DATA_UPDATE
...
### Scenario #4 : Shipment update on a shipment that already had the keyword as inherited RISK_DATA_UPDATE (column unchanged)
...
### Scenario #5 : Shipment update on an inherited RISK_DATA_UPDATE hit where the matched column DID change

Stickiness must hold even when the shipment edit actually touches `c1`. Because
`k1` no longer exists in `kl1`, there is nothing to re-evaluate as a
...
### Scenario #6 : Reverse ordering — SHIPMENT_UPDATE first, then the keyword is removed from the list
...
### Scenario #7 : Multiple keywords in the same list, partial removal
...
### Scenario #8 : Keyword re-added to the list (resurrection)
...
the new list version, and the inherited record is superseded (archived, no
longer shown as a deleted hit).
...
### Scenario #9 : Matched word re-introduced after a SHIPMENT_UPDATE
...
a fresh active hit and the inherited record is cleared.
...
### Scenario #10 : No-op shipment update must not trigger inheritance
...
| # | Scenario | Final state of `k1` | Key assertion |
|---|---|---|---|
| 1 | Regular hit | active | baseline active hit |
| 2 | Shipment update removes keyword | `SHIPMENT_UPDATE` / `klv1` | ship icon |
| 3 | Keyword removed from list | `RISK_DATA_UPDATE` / `klv1` | info icon, old version kept |
| 4 | Shipment update, column unchanged, over RISK_DATA_UPDATE | `RISK_DATA_UPDATE` / `klv1` | **does not flip** to SHIPMENT_UPDATE |
| 5 | Shipment update, column changed, over RISK_DATA_UPDATE | `RISK_DATA_UPDATE` / `klv1` | stickiness holds even when column moves |
| 6 | SHIPMENT_UPDATE first, then list removal | `SHIPMENT_UPDATE` / `klv1` | **does not flip** to RISK_DATA_UPDATE |
| 7 | Partial removal, two keywords | `k1` inherited, `k2` active | mixed state on one shipment |
| 8 | Keyword re-added to list | active / `klv3` | inherited record superseded |
| 9 | Word re-introduced after SHIPMENT_UPDATE | active / `klv1` | inherited record cleared |
| 10 | No-op shipment update | active / `klv1` | inheritance not triggered |
